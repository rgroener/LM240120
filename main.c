...
void glcd_write_bmp(uint16_t posx, uint8_t posy, uint16_t with, uint8_t hight, char bmp[])// writes string to display at current cursor position.
{
	uint16_t x=1;
	uint16_t linewrap=0;
...
int main(void)
{
	...
	for(xx=0;xx<120;xx++)
	{
		...
		*/	
		...
		glcd_write_count(123,10);
		
		glcd_goto_xy(0,2);
		glcd_write_string("Woodstock");
		glcd_write_bmp(5,5,48,48,&bild[0]);
